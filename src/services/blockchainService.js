...
// Import the blockchain configuration
...
class BlockchainService {
  ...
  async updateGameStatsDetailed(gamesPlayed, gamesWon, chainReactions, eliminations, longestChain, perfectGames) {
    try {
      // Use the new separated functions
      await this.updateGameStats(gamesPlayed, gamesWon);
      await this.updateAdvancedStats(chainReactions, eliminations, longestChain, perfectGames);
    } catch (error) {
      console.error('Failed to update detailed game stats:', error);
      this.handleError(error);
      throw error;
    }
  }
